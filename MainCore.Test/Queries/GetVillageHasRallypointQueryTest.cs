...
{
    public class GetVillageHasRallypointQueryTest
    {
        [Fact]
        public async Task QueryShouldRun()
        {
            using var context = new FakeDbContextFactory().CreateDbContext();
            var handleBehavior = new GetVillageHasRallypointQuery.HandleBehavior(context);
            var accountId = new AccountId(1);
            var query = new GetVillageHasRallypointQuery.Query(accountId);
